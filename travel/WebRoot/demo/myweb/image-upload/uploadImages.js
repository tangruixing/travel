/**
 * Created by king on 2015/6/16.
 */

var uploadImages = function(uploader, imageId, fileNumLimit) {
	this.wrap = "#" + uploader;
	this.startPicker = "#" + uploader + "_startPicker";

	this.nextPicker = "#" + uploader + "_nextPicker";
	this.dndArea = "#" + uploader + "_dndArea";
	this.imageId = $("#" + imageId);
	this.fileNumLimit = fileNumLimit || 300;

	this.uploaderSwf = '../../dist/Uploader.swf';
	this.expressInstallSwf = './expressInstall.swf';
	this.imageList = [];
	this.server = sy.contextPath = "/travel/UploadImageServlet";
	this.init();
}
uploadImages.prototype = {
	...
	init : function() {
		...
		if (!WebUploader.Uploader.support('flash') && WebUploader.browser.ie) {
			...
			if (flashVersion) {
				(function(container) {
					...
					var swf = _this.expressInstallSwf;
					// insert flash object
		...
		uploader = WebUploader.create({
			...
			/*
			 * server:
			 * 'http://localhost:8080/travel/jslib/ueditor1_2_6_1-utf8-jsp/jsp/imageUp.jsp',
			 */// runtimeOrder: 'flash',
			// accept: {
			// title: 'Images',
		...
		function setState(val) {
			...
			state = val;
			var $nextPikcer = $(_this.nextPicker);
			switch (state) {
			case 'pedding':
			...
			case 'ready':
				...
				$nextPikcer.removeClass('element-invisible');
				$queue.show();
			...
			case 'uploading':
				$nextPikcer.addClass('element-invisible');
				$progress.show();
				$upload.text('暂停上传');
				break;
